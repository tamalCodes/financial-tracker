...
|---|---|---|
| **Auth** — canonical `/auth` login/signup, Supabase email/password + Google/Apple OAuth, cookie-based server auth; signup captures full name + bank balance; successful password login writes `profiles.last_login_at`; greeting shows first name; log out via avatar menu; login/signup support Kharcha light and dark themes | `features/auth.md` | `src/app/api/auth/*`, `src/app/auth/callback/route.ts`, `src/app/(auth)/{layout.tsx,auth/page.tsx,login/page.tsx,signup/page.tsx}`, `src/app/(app)/dashboard/page.tsx`, `src/app/{globals,layout}.tsx`, `public/{icon.svg,manifest.json}`, `lib/api/schemas.ts`, `lib/supabase/{auth,cookies}.ts`, `features/auth/{AuthContext,identity}.ts(x)`, `features/auth/components/AuthForm.tsx`, `supabase/config.toml`, `features/dashboard/{mobile/MobileHome,mobile/AvatarMenu,desktop/DesktopHome}.tsx`, `supabase/migrations/{008_profiles_full_name,010_profiles_last_login}.sql`, `src/proxy.ts` |
...
| **Landing page** — public Kharcha marketing page at `/`; `/auth` CTA routing; sitemap + robots metadata; centred Luminous Ledger hero with an immediate three-metric monthly pulse, natural phone-in-hand product proof, gentle one-time reveals, low-amplitude device drift and subtle CSS edge atmosphere, premium warm editorial story, responsive phone/tablet scroll-sheet parallax, fully framed freedom artwork, and adaptive footer information architecture | `features/landing.md` | `src/app/{page,sitemap,robots}.ts(x)`, `src/app/landing/{page.tsx,HeroPhone.tsx,LandingMotion.tsx,StoryRotator.tsx,landing.module.css}`, `public/landing/{hero-phone-hand-v5.png,freedom.jpg,freedom-cutout.png,testimonial-{meera,aarav,riya}.png}`, `.env.example`, `package.json`, `src/app/{layout,globals}.css` |
...
| `LANDING_DESIGN_SYSTEM.md` | **AI-readable public/editorial language** for `/`: brand ideology, Fraunces/Manrope typography, warm palette, adaptive liquid glass, motion grammar, subtle gamification, responsive composition, section anatomy, footer, content voice, accessibility, and agent acceptance checklist |
