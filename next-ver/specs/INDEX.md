...
|---|---|---|
| **Auth** — email/password via Supabase, cookie-based server auth; signup captures full name + bank balance; successful login writes `profiles.last_login_at`; greeting shows first name; log out via avatar menu; login/signup support light and dark mode | `features/auth.md` | `src/app/api/auth/*`, `src/app/{globals,layout}.tsx`, `public/{icon.svg,manifest.json}`, `lib/api/schemas.ts`, `lib/supabase/{auth,cookies}.ts`, `features/auth/{AuthContext,identity}.ts(x)`, `features/auth/components/AuthForm.tsx`, `src/app/(auth)/layout.tsx`, `features/dashboard/{mobile/MobileHome,desktop/DesktopHome}.tsx`, `supabase/migrations/{008_profiles_full_name,010_profiles_last_login}.sql`, `src/middleware.ts`, `features/dashboard/mobile/AvatarMenu.tsx` |
