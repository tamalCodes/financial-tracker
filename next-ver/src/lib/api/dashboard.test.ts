 ...
 * (select/order/eq/lte) that resolves to the table's rows filtered by the recorded
 * eq/lte predicates. user_id is ignored (single-user fixtures).
 */
...
describe("cumulativeLeftInBank", () => {
  it("sums earned − (expenses + paid bills) − invested across all months ≤ current", async () => {
    const db = mockSupabase(fixtures);
    // June: earned 50000 − (expenses 13000 + paid bills 1500) − invested 5000 = 30500
...
describe("loadDashboardData — per-month summary", () => {
  it("tiles reflect the current month only; spent includes paid bills", async () => {
    ...
    expect(summary.earned).toBe(30000); // June credits only
    ...
    expect(summary.leftInBank).toBe(30500); // cumulative
