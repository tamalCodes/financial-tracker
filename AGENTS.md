...
- Write project artifacts into the repo, for example under `specs/`.

## Spec-first (read before coding)
- **Before writing or searching code, consult `specs/` first.**
- Start at `specs/INDEX.md`, grep it, match the task to a feature row, open that spec, then jump straight to the code files it lists ("Reverse-engineered from").
